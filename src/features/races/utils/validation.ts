...
function validateSingleRace(race: any): boolean {
  ...
  return requiredFields.every(field => {
    ...
    switch (field) {
      ...
      case 'skillBonuses':
        return (
          Array.isArray(race[field]) && race[field].every(validateSkillBonus)
        )
      case 'racialSpells':
        return (
          Array.isArray(race[field]) && race[field].every(validateRacialSpell)
        )
      case 'keywords':
...
function validateStartingStats(stats: any): boolean {
  ...
  return requiredStats.every(
    stat => typeof stats[stat] === 'number' && stats[stat] >= 0
  )
}
...
function validateSkillBonus(bonus: any): boolean {
  return (
    bonus && typeof bonus.skill === 'string' && typeof bonus.bonus === 'number'
  )
}
...
export function handleMissingData(race: Race): Race {
  ...
  if (!race.name || !race.edid || !race.description) {
    console.warn(
      `Race data incomplete for ${race.name || 'unknown'}, using defaults`
    )
  }
  ...
  races.forEach((race, index) => {
    try {
      ...
      if (transformed.effects.length === 0 && race.racialSpells.length > 0) {
        errors.push(
          `Race ${index}: No effects transformed from ${race.racialSpells.length} spells`
        )
      }
    } catch (error) {
