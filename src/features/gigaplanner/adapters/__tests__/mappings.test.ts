...
describe('Standing Stone EDID Mappings', () => {
  ...
  describe('getStandingStoneEdid', () => {
    it('should return correct EDID for known standing stone names', () => {
      ...
      expect(getStandingStoneEdid('Warrior')).toBe(
        'REQ_Ability_Birthsign_Warrior'
      )
      ...
      expect(getStandingStoneEdid('Apprentice')).toBe(
        'REQ_Ability_Birthsign_Apprentice'
      )
      expect(getStandingStoneEdid('Atronach')).toBe(
        'REQ_Ability_Birthsign_Atronach'
      )
      expect(getStandingStoneEdid('Ritual')).toBe(
        'REQ_Ability_Birthsign_Ritual'
      )
      ...
      expect(getStandingStoneEdid('Shadow')).toBe(
        'REQ_Ability_Birthsign_Shadow'
      )
      expect(getStandingStoneEdid('Tower')).toBe('REQ_Ability_Birthsign_Tower')
      expect(getStandingStoneEdid('Serpent')).toBe(
        'REQ_Ability_Birthsign_Serpent'
      )
    })
  ...
  describe('getStandingStoneNameFromEdid', () => {
    it('should return correct standing stone name for known EDIDs', () => {
      ...
      expect(
        getStandingStoneNameFromEdid('REQ_Ability_Birthsign_Warrior')
      ).toBe('Warrior')
      expect(getStandingStoneNameFromEdid('REQ_Ability_Birthsign_Lady')).toBe(
        'Lady'
      )
      expect(getStandingStoneNameFromEdid('REQ_Ability_Birthsign_Lord')).toBe(
        'Lord'
      )
      expect(getStandingStoneNameFromEdid('REQ_Ability_Birthsign_Steed')).toBe(
        'Steed'
      )
      expect(getStandingStoneNameFromEdid('REQ_Ability_Birthsign_Mage')).toBe(
        'Mage'
      )
      expect(
        getStandingStoneNameFromEdid('REQ_Ability_Birthsign_Apprentice')
      ).toBe('Apprentice')
      expect(
        getStandingStoneNameFromEdid('REQ_Ability_Birthsign_Atronach')
      ).toBe('Atronach')
      expect(getStandingStoneNameFromEdid('REQ_Ability_Birthsign_Ritual')).toBe(
        'Ritual'
      )
      expect(getStandingStoneNameFromEdid('REQ_Ability_Birthsign_Thief')).toBe(
        'Thief'
      )
      expect(getStandingStoneNameFromEdid('REQ_Ability_Birthsign_Lover')).toBe(
        'Lover'
      )
      expect(getStandingStoneNameFromEdid('REQ_Ability_Birthsign_Shadow')).toBe(
        'Shadow'
      )
      expect(getStandingStoneNameFromEdid('REQ_Ability_Birthsign_Tower')).toBe(
        'Tower'
      )
      expect(
        getStandingStoneNameFromEdid('REQ_Ability_Birthsign_Serpent')
      ).toBe('Serpent')
    })
...
describe('Perk EDID Mappings', () => {
  describe('PERK_NAME_TO_EDID', () => {
    it('should contain expected perk mappings', () => {
      expect(PERK_NAME_TO_EDID['Craftsmanship']).toBe('PerkCraftsmanship')
      expect(PERK_NAME_TO_EDID['Advanced Blacksmithing']).toBe(
        'PerkAdvancedBlacksmithing'
      )
      expect(PERK_NAME_TO_EDID['Elven Smithing']).toBe('PerkElvenSmithing')
    })

    it('should have multiple perk mappings', () => {
      expect(Object.keys(PERK_NAME_TO_EDID).length).toBeGreaterThan(0)
    })
  })

  describe('getPerkEdid', () => {
    it('should return correct EDID for known perk names', () => {
      expect(getPerkEdid('Craftsmanship')).toBe('PerkCraftsmanship')
      expect(getPerkEdid('Advanced Blacksmithing')).toBe(
        'PerkAdvancedBlacksmithing'
      )
      expect(getPerkEdid('Elven Smithing')).toBe('PerkElvenSmithing')
    })

    it('should return the input name for unknown perk names', () => {
      expect(getPerkEdid('UnknownPerk')).toBe('UnknownPerk')
      expect(getPerkEdid('')).toBe('')
      expect(getPerkEdid('craftsmanship')).toBe('craftsmanship') // Case sensitive
    })
  })

  describe('getPerkNameFromEdid', () => {
    it('should return correct perk name for known EDIDs', () => {
      expect(getPerkNameFromEdid('PerkCraftsmanship')).toBe('Craftsmanship')
      expect(getPerkNameFromEdid('PerkAdvancedBlacksmithing')).toBe(
        'Advanced Blacksmithing'
      )
      expect(getPerkNameFromEdid('PerkElvenSmithing')).toBe('Elven Smithing')
    })

    it('should return null for unknown EDIDs', () => {
      expect(getPerkNameFromEdid('UnknownPerk')).toBeNull()
      expect(getPerkNameFromEdid('')).toBeNull()
      expect(getPerkNameFromEdid('perkcraftsmanship')).toBeNull() // Case sensitive
    })
  })

  describe('bidirectional mapping consistency', () => {
    it('should maintain consistency between getPerkEdid and getPerkNameFromEdid', () => {
      const perkNames = Object.keys(PERK_NAME_TO_EDID)

      for (const perkName of perkNames) {
        const edid = getPerkEdid(perkName)
        const mappedPerkName = getPerkNameFromEdid(edid)
        expect(mappedPerkName).toBe(perkName)
      }
    })

    it('should maintain consistency in reverse direction', () => {
      const edids = Object.values(PERK_NAME_TO_EDID)

      for (const edid of edids) {
        const perkName = getPerkNameFromEdid(edid)
        const mappedEdid = getPerkEdid(perkName!)
        expect(mappedEdid).toBe(edid)
      }
    })
  })
...
describe('Skill Mappings', () => {
  describe('SKILL_NAMES', () => {
    it('should contain all expected skill names', () => {
      ...
      expect(SKILL_NAMES).toEqual(expectedSkills)
    })

    it('should have 20 skill names', () => {
      expect(SKILL_NAMES).toHaveLength(20)
    })
  })

  describe('getSkillName', () => {
    it('should return correct skill name for valid indices', () => {
      expect(getSkillName(0)).toBe('Smithing')
      expect(getSkillName(5)).toBe('Marksman')
      expect(getSkillName(19)).toBe('Traits')
    })

    it('should return "Unknown" for invalid indices', () => {
      expect(getSkillName(-1)).toBe('Unknown')
      expect(getSkillName(20)).toBe('Unknown')
      expect(getSkillName(999)).toBe('Unknown')
    })
  })

  describe('getSkillIndex', () => {
    it('should return correct index for valid skill names', () => {
      expect(getSkillIndex('Smithing')).toBe(0)
      expect(getSkillIndex('Marksman')).toBe(5)
      expect(getSkillIndex('Destruction')).toBe(14)
      expect(getSkillIndex('Traits')).toBe(19)
    })

    it('should return -1 for invalid skill names', () => {
      expect(getSkillIndex('UnknownSkill')).toBe(-1)
      expect(getSkillIndex('')).toBe(-1)
      expect(getSkillIndex('smithing')).toBe(-1) // Case sensitive
    })
  })

  describe('bidirectional mapping consistency', () => {
    it('should maintain consistency between getSkillName and getSkillIndex', () => {
      for (let i = 0; i < SKILL_NAMES.length; i++) {
        const skillName = getSkillName(i)
        const skillIndex = getSkillIndex(skillName)
        expect(skillIndex).toBe(i)
      }
    })
    ...
    it('should maintain consistency in reverse direction', () => {
      for (const skillName of SKILL_NAMES) {
        const skillIndex = getSkillIndex(skillName)
        const mappedSkillName = getSkillName(skillIndex)
        expect(mappedSkillName).toBe(skillName)
      }
    })
  })
})
