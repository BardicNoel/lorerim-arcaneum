...
export function usePerkPlan(tree: PerkTree | undefined) {
  ...
  const getAllDescendants = (
    ...
    tree.perks.forEach(perk => {
      if (
        perk.prerequisites?.perks?.some(
          prereq => prereq.type === 'PERK' && prereq.id === perkId
        )
      ) {
      ...
      setPerkPlan(prev => {
        ...
        if (perkIndex >= 0) {
          ...
          if (perk.ranks > 1) {
            ...
            if (nextRank === 0) {
              ...
              if (hasOtherParents) {
                ...
                console.log(
                  `Cycling ${perk.perkName} to rank 0 (kept due to other parents)`
                )
                return {
                  ...prev,
                  selectedPerks: {
                ...
                const selectedPerksToRemove = skillPerks.filter(p =>
                  perksToRemove.has(p.perkId)
                )
                const newSkillPerks = skillPerks.filter(
                  p => !perksToRemove.has(p.perkId)
                )

                console.log(
                  `Cycling to rank 0, removing ${selectedPerksToRemove.length} perks:`,
                  selectedPerksToRemove.map(p => p.perkName)
                )
                return {
                  ...prev,
                  selectedPerks: {
            ...
            console.log(
              `Removing ${selectedPerksToRemove.length} perks:`,
              selectedPerksToRemove.map(p => p.perkName)
            )
            return {
              ...prev,
          ...
          const newPerks = Array.from(perksToAdd)
            ...
            .filter(
              (perk): perk is NonNullable<typeof perk> => perk !== undefined
            )
            .map(perk => {
              // Check if this perk is already selected (might happen with multi-parent perks)
              const existingPerk = skillPerks.find(
                p => p.perkId === perk.perkId
              )
              ...
              if (existingPerk) {
                ...
                const shouldHaveRank =
                  perk.perkId === perkId ||
                  (perk.ranks > 1 && perksToAdd.has(perk.perkId))
                return {
                  ...perk,
                  selected: true,
          ...
          const perksToUpdate = newPerks.filter(perk =>
            existingPerkIds.has(perk.perkId)
          )
          const newPerksToAdd = newPerks.filter(
            perk => !existingPerkIds.has(perk.perkId)
          )

          // Update existing perks with new data
          const updatedSkillPerks = skillPerks.map(existingPerk => {
            const updateData = perksToUpdate.find(
              p => p.perkId === existingPerk.perkId
            )
            return updateData
              ? { ...existingPerk, ...updateData }
              : existingPerk
          })

          const newSkillPerks = [...updatedSkillPerks, ...newPerksToAdd]
          console.log(
            `Adding ${newPerksToAdd.length} new perks and updating ${perksToUpdate.length} existing perks:`,
            newPerksToAdd.map(p => p.perkName)
          )
          return {
            ...prev,
            selectedPerks: { ...prev.selectedPerks, [skill]: newSkillPerks },
  ...
  const updatePerkRank = useCallback(
    (perkId: string, newRank: number) => {
      ...
      setPerkPlan(prev => {
        ...
        if (perkIndex >= 0) {
          ...
          const validRank =
            perk.ranks > 1 ? Math.max(0, Math.min(newRank, perk.ranks)) : 0
          ...
          console.log(
            `Attempted to update rank for unselected perk: ${perk.perkName}`
          )
          return prev
        }
      })
