...
export function SkillsPage() {
  ...
  const { skills, loading, error, adapter, refreshSkills } = useSkillData()
  const { 
    selectedSkillId, 
    setSelectedSkillId, 
    assignSkill, 
    unassignSkill 
  } = useSkillState(adapter)
  const { 
    searchQuery, 
    setSearchQuery, 
    selectedCategory,
  ...
  const handleAssignMajor = useCallback((skillId: string) => {
  ...
  const handleAssignMinor = useCallback((skillId: string) => {
  ...
  const handleRemoveAssignment = useCallback((skillId: string) => {
