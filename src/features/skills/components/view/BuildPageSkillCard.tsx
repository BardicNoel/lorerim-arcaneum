...
import { useSkillData } from '../../adapters'
...
export function BuildPageSkillCard({ className }: BuildPageSkillCardProps) {
  ...
  const otherPerkedSkills = useMemo(() => {
    const majorSkillIds = selectedMajorSkills.map(s => s.edid)
    const minorSkillIds = selectedMinorSkills.map(s => s.edid)
  ...
  const { selectedPerkTree, loading: perkLoading } = usePerkData(selectedSkillId)

  const renderSkillCard = (skill: any, type: 'major' | 'minor' | 'perked' | 'assigned') => {
    ...
    <>
      ...
      >
          ...
          <div className="space-y-4">
            ...
            {selectedMajorSkills.length > 0 && (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                {selectedMajorSkills.map((skill) => renderSkillCard(skill, 'major'))}
              </div>
            )}
          </div>
           ...
           <div className="space-y-4">
             ...
             {selectedMinorSkills.length > 0 && (
               <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                 {selectedMinorSkills.map((skill) => renderSkillCard(skill, 'minor'))}
               </div>
             )}
           </div>
                       ...
                       {/* Other Perked Skills */}
            <div className="space-y-4">
              <h4 className="text-md font-medium text-foreground">Other Perked Skills</h4>
              ...
              {otherPerkedSkills.length > 0 && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                  {otherPerkedSkills.map((skill) => renderSkillCard(skill, 'perked'))}
                </div>
              )}
            </div>
          ...
          {assignedSkillsWithoutPerks.length > 0 && (
            <div className="space-y-4">
              <h4 className="text-md font-medium text-foreground">Assigned Skills (No Perks)</h4>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                {assignedSkillsWithoutPerks.map((skill) => renderSkillCard(skill, 'assigned'))}
              </div>
            </div>
          )}
        </div>
        ...
        <div className="mt-6 p-4 bg-muted/50 rounded-lg">
          <p className="text-sm text-muted-foreground">
            Click on any skill card to view and manage its perk tree. Skill levels show the minimum level required based on your perk selections.
          </p>
        </div>
      </SelectionCardShell>
