...
export function AccordionRacesPage() {
  ...
  // Use MVA adapters instead of direct data fetching
  const { races, isLoading, error } = useRaceData()
  const { setSearchQuery, filteredRaces } = useRaceFilters({ races })
  const { searchCategories } = useRaceComputed({
    races,
    selectedRaceId: null,
  })
  ...
  const filteredRacesWithTags = filteredRaces.filter(race => {
  ...
  // Use the search query from the adapter for fuzzy search
  useEffect(() => {
    setSearchQuery(fuzzySearchQuery)
  }, [fuzzySearchQuery, setSearchQuery])

  // Convert to PlayerCreationItem format, but include originalRace for details
  const displayItems = filteredRacesWithTags.map(race => {
