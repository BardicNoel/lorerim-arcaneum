...
function measureNodeSizes(
  ...
  console.log(
    'Node sizes measured:',
    Array.from(sizes.entries()).map(([id, width]) => `${id}: ${width}px`)
  )
...
function calculateTreeWidth(
  ...
  console.log(
    `Tree with ${treePerks.length} perks: max ${maxPerksInLevel} perks in a level, width: ${totalWidth}px`
  )
...
function scalePositions(
  ...
  const totalWidth =
    treeWidths.reduce((sum, width) => sum + width, 0) +
    (trees.length - 1) * treeSpacing

  console.log(
    `Total layout width: ${totalWidth}px (${treeWidths.join(' + ')} + ${(trees.length - 1) * treeSpacing} spacing)`
  )
  ...
  trees.forEach((treePerks, treeIndex) => {
    ...
    console.log(
      `Tree ${treeIndex + 1} Y range: ${minY} to ${maxY} (range: ${yRange})`
    )
    console.log(
      `Tree ${treeIndex + 1} X range: ${minX} to ${maxX} (range: ${xRange})`
    )
    ...
    treePerks.forEach(perk => {
      ...
      if (parents.length > 0) {
        ...
        if (hasParentAtSameLevel) {
          ...
          console.log(
            `${perk.edid}: Same Y as parent, shifting up by ${halfStepOffset}px to Y=${canvasY}`
          )
        }
      }

      console.log(
        `${perk.edid}: grid (${perk.position.x},${perk.position.y}), canvas (${canvasX.toFixed(0)},${canvasY.toFixed(0)})`
      )
...
function centerSubtrees(
  ...
  const centerParentUnderChildren = (parentId: string): boolean => {
    ...
    if (childCount > 0) {
      ...
      console.log(
        `🔍 Centering parent ${parentId} under ${childCount} children: current center=${currentCenterX.toFixed(0)}, children center=${childrenCenterX.toFixed(0)}, move=${moveDistance.toFixed(0)}, max=${maxMoveDistance}`
      )

      if (moveDistance <= maxMoveDistance) {
        console.log(
          `  ✅ Centering parent ${parentId} under ${childCount} children: x=${parent.x.toFixed(0)} -> ${newParentX.toFixed(0)}`
        )
        parent.x = newParentX
        ...
        console.log(
          `  ❌ Skipping parent ${parentId}: move too large (${moveDistance.toFixed(0)}px > ${maxMoveDistance}px)`
        )
      }
    }
  ...
  const centerChildrenUnderParent = (parentId: string): boolean => {
    ...
    console.log(
      `🔍 Centering children under ${parentId}: parent center=${parentCenterX.toFixed(0)}, total width=${totalWidth.toFixed(0)}, start X=${currentX.toFixed(0)}`
    )
    ...
    validChildren.forEach(childId => {
      ...
      if (child) {
        ...
        console.log(
          `  📍 Child ${childId}: current=${child.x.toFixed(0)}, target=${newChildX.toFixed(0)}, move=${moveDistance.toFixed(0)}, max=${maxMoveDistance}`
        )

        if (moveDistance <= maxMoveDistance) {
          console.log(
            `  ✅ Centering child ${childId} under parent ${parentId}: x=${child.x.toFixed(0)} -> ${newChildX.toFixed(0)}`
          )
          child.x = newChildX
          ...
          console.log(
            `  ❌ Skipping child ${childId}: move too large (${moveDistance.toFixed(0)}px > ${maxMoveDistance}px)`
          )
        }
  ...
  const spaceSiblingsEvenly = (parentId: string): boolean => {
    ...
    validChildren.forEach(childId => {
      ...
      if (child) {
        ...
        if (moveDistance <= maxMoveDistance) {
          console.log(
            `Spacing sibling ${childId} around parent ${parentId}: x=${child.x.toFixed(0)} -> ${newChildX.toFixed(0)}`
          )
          child.x = newChildX
          child.originalX = newChildX
          spacedCount++
  ...
  const logDebugNodes = (phase: string) => {
    ...
    debugNodes.forEach(nodeId => {
      ...
      if (node) {
        console.log(
          `  ${nodeId}: x=${node.x.toFixed(0)}, y=${node.y.toFixed(0)}`
        )
      }
    })
  }
  ...
  while (parentQueue.length > 0) {
    ...
    parents.forEach(parentId => {
      if (!processedParents.has(parentId)) {
        ...
        const validParentChildren = parentChildren.filter(childId =>
          nodeMap.has(childId)
        )
        const allChildrenProcessed = validParentChildren.every(childId =>
          processedParents.has(childId)
        )
  ...
  console.log(
    `Comprehensive centering completed: ${totalCentered} adjustments made`
  )
...
function resolveCollisions(
  ...
  const shiftSubtree = (rootId: string, deltaX: number): void => {
    ...
    console.log(
      `Shifted subtree rooted at ${rootId} by ${deltaX.toFixed(0)}px (${subtreeNodes.length} nodes)`
    )
  }
  ...
  for (let i = 0; i < sortedRoots.length - 1; i++) {
    ...
    if (overlap > 0) {
      ...
      console.log(
        `Resolving overlap between subtrees ${rootA} and ${rootB}: shifting by ${overlap.toFixed(0)}px`
      )
...
function layoutPerkTree(
  ...
  try {
    ...
    console.log(
      ...
      trees.map(tree => tree.map(p => p.edid.split('_').pop()))
    )

    // 2️⃣ For each tree, calculate depths and build hierarchy
    console.log('=== STEP 2: BUILDING HIERARCHY ===')
    const treeHierarchies = trees.map(treePerks =>
      buildTreeHierarchy(treePerks)
    )
    ...
    const treeLayouts = treeHierarchies.map((hierarchy, treeIndex) => {
      console.log(
        `Layouting tree ${treeIndex + 1} with ${hierarchy.nodes.size} nodes`
      )
    ...
    console.log(
      'Final positions:',
      forceAdjustedNodes.map(
        n => `${n.id}: (${n.x.toFixed(0)}, ${n.y.toFixed(0)})`
      )
    )
...
function buildTreeHierarchy(treePerks: PerkRecord[]) {
  ...
  console.log(
    `  Tree has ${roots.length} roots:`,
    roots.map(id => id.split('_').pop())
  )
      ...
      if (node.parents.length > 1) {
        ...
        if (parentDepths.length > 0) {
          ...
          if (node.depth !== expectedDepth) {
            console.log(
              `    ${nodeId.split('_').pop()}: depth ${node.depth} -> ${expectedDepth} (parents: ${node.parents.map(p => p.split('_').pop()).join(', ')})`
            )
  ...
  console.log(
    '  Third pass: Ensuring children are at higher depth than parents'
  )
  nodes.forEach((node, nodeId) => {
    node.children.forEach(childId => {
      ...
      if (child && child.depth <= node.depth) {
        ...
        console.log(
          `    ${childId.split('_').pop()}: depth ${child.depth} -> ${newDepth} (child of ${nodeId.split('_').pop()} at depth ${node.depth})`
        )
...
function layoutSingleTree(
  ...
  console.log(
    `Layout single tree with ${nodes.size} nodes and ${roots.length} roots`
  )
  ...
  const hasCircularReferences = layoutNodes.some(node => {
    try {
      ...
      const testResult = calculateSubtreeWidth(
        node.id,
        nodes,
        config,
        testVisited
      ...
      console.log(
        `Circular reference detected in tree, will use original positions for ${node.id}`
      )
      return true
    }
  })
  ...
  if (hasCircularReferences) {
    ...
    console.log(
      `Tree has ${layoutNodes.length} nodes, checking original positions...`
    )
    ...
    layoutNodes.forEach(node => {
      ...
      if (hierarchyNode && hierarchyNode.perk) {
        ...
        console.log(
          `${node.id.split('_').pop()}: original pos (${hierarchyNode.perk.position.x}, ${hierarchyNode.perk.position.y}) -> scaled (${originalX.toFixed(0)}, ${originalY.toFixed(0)})`
        )
  ...
  console.log(
    `Found ${rootNodes.length} root nodes: ${rootNodes.map(n => n.id.split('_').pop()).join(', ')}`
  )
  ...
  const visited = new Set<string>()
  const queue: { node: LayoutNode; depth: number }[] = rootNodes.map(node => ({
    node,
    depth: 0,
  }))
          ...
          if (children.length > 0) {
            ...
            if (parentNode) {
              ...
              const childPositions = children.map(child => {
                const subtreeWidth = calculateSubtreeWidth(
                  child.id,
                  nodes,
                  config
                )
              ...
              const totalWidth = childPositions.reduce(
                (sum, { subtreeWidth }) => sum + subtreeWidth,
                0
              )
...
function applyMildForces(
  ...
  for (let iteration = 0; iteration < 5; iteration++) {
    ...
    for (let i = 0; i < layoutNodes.length; i++) {
      for (let j = i + 1; j < layoutNodes.length; j++) {
        ...
        if (distance < minDistance && distance > 0) {
          ...
          if (iteration === 0) {
            console.log(
              `  💫 Repulsion between ${nodeA.id.split('_').pop()} and ${nodeB.id.split('_').pop()}: force=(${forceX.toFixed(2)}, ${forceY.toFixed(2)})`
            )
          }
        }
      }
    ...
    console.log(
      `  Iteration ${iteration + 1}: Applied repulsion to ${totalForceApplied} node pairs`
    )
  ...
  layoutNodes.forEach(node => {
    ...
    if (Math.abs(finalY - node.y) > 1 || Math.abs(newX - node.x) > 1) {
      console.log(
        `  🎯 Attraction for ${node.id.split('_').pop()}: x=${node.x.toFixed(0)}->${newX.toFixed(0)}, y=${node.y.toFixed(0)}->${finalY.toFixed(0)} (depth constraint: ${expectedY.toFixed(0)})`
      )
    }
...
function applyCircularTreeForces(
  layoutNodes: LayoutNode[],
  config: LayoutConfig
): LayoutNode[] {
  ...
  while (totalTouching > 0 && iteration < maxIterations) {
      ...
      for (let j = i + 1; j < layoutNodes.length; j++) {
        ...
        if ((overlapX > 0 && overlapY > 0) || needsVerticalSeparation) {
          ...
          if (iteration === 1) {
            if (overlapX > 0 && overlapY > 0) {
              console.log(
                `  💥 Vertical repulsion (overlap) between ${nodeA.id.split('_').pop()} and ${nodeB.id.split('_').pop()}: forceY=${forceY.toFixed(2)}`
              )
            } else {
              console.log(
                `  📏 Vertical separation between ${nodeA.id.split('_').pop()} and ${nodeB.id.split('_').pop()}: forceY=${forceY.toFixed(2)}`
              )
            }
          }
        }
    ...
    console.log(
      `  Iteration ${iteration}: ${totalTouching} touching/close pairs, max force: ${maxForceApplied.toFixed(2)}`
    )

    // Stop if no nodes are touching or too close
    if (totalTouching === 0) {
      console.log(
        `  No more touching or close nodes after iteration ${iteration}`
      )
      break
    }
  }
  ...
  if (iteration >= maxIterations) {
    console.log(
      `  Warning: Reached maximum iterations (${maxIterations}) with ${totalTouching} still touching/close`
    )
  }
  ...
  layoutNodes.forEach(node => {
    ...
    if (Math.abs(newY - node.y) > 1) {
      console.log(
        `  🎯 Vertical attraction for ${node.id.split('_').pop()}: y=${node.y.toFixed(0)}->${newY.toFixed(0)}`
      )
    }
...
function positionTreesAsSiblings(
  ...
  console.log(
    `Overall root range: ${allMinRootX.toFixed(0)} to ${allMaxRootX.toFixed(0)} (width: ${totalRootWidth.toFixed(0)})`
  )
  ...
  treeAnalyses.forEach((analysis, index) => {
    console.log(
      `  Tree ${index + 1}: root range ${analysis.minRootX.toFixed(0)}-${analysis.maxRootX.toFixed(0)}, tree width=${analysis.treeWidth.toFixed(0)}`
    )
    console.log(
      `    Root nodes: ${analysis.rootNodes.map(n => n.id.split('_').pop()).join(', ')}`
    )

    // Calculate scale factor to normalize root positions
    const scaleFactor =
      totalRootWidth > 0 ? analysis.rootWidth / totalRootWidth : 1
