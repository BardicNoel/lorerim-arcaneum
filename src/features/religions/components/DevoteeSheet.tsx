...
const effectIcons: Record<string, React.ReactNode> = {
  ...
  Conjuration: <BookOpen className="h-4 w-4 text-purple-500" />,
...
export function DevoteeSheet({
  ...
  const devoteeBoon = religion.boon2
  const effectsCount = devoteeBoon?.effects?.length || 0

  if (!devoteeBoon || effectsCount === 0) {
    return null
  }
  ...
  return (
    <ResponsivePanel
      ...
      title={`Devotee of ${religion.name}`}
      description="Ultimate powers granted to devoted worshippers"
    >
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <AddToBuildSwitchSimple
            item={{
              id: `devotee-${religion.name}`,
              name: `Devotee of ${religion.name}`,
              category: 'Devotee',
              description: devoteeBoon.effects[0]?.effectDescription || '',
              effects: devoteeBoon.effects.map(effect => ({
        ...
        {/* Devotee Powers */}
        <div>
          <H3 className="text-lg font-semibold mb-3">Devotee Powers</H3>
        ...
        <div className="pt-4 border-t border-border">
          <H3 className="text-lg font-semibold mb-3">
            About Devotion to {religion.name}
          </H3>
          <div className="space-y-3">
            <div>
