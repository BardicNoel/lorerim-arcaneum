  ...
  persist(
    (set, get) => ({
      ...
      addTag: (tag: SelectedTag) => {
        set((state) => {
          ...
          if (tag.category === TAG_CATEGORIES.TYPES) {
            newActiveFilters.types = [...newActiveFilters.types, tag.value]
          } else if (tag.category === TAG_CATEGORIES.CATEGORIES) {
            newActiveFilters.categories = [...newActiveFilters.categories, tag.value]
          } else if (tag.category === TAG_CATEGORIES.FUZZY_SEARCH || tag.category === TAG_CATEGORIES.SEARCH_ALL) {
            newActiveFilters.tags = [...newActiveFilters.tags, tag.value]
          }
      ...
      removeTag: (tagId: string) => {
        set((state) => {
          ...
          if (tagToRemove.category === TAG_CATEGORIES.TYPES) {
            newActiveFilters.types = newActiveFilters.types.filter(t => t !== tagToRemove.value)
          } else if (tagToRemove.category === TAG_CATEGORIES.CATEGORIES) {
            newActiveFilters.categories = newActiveFilters.categories.filter(c => c !== tagToRemove.value)
          } else if (tagToRemove.category === TAG_CATEGORIES.FUZZY_SEARCH || tagToRemove.category === TAG_CATEGORIES.SEARCH_ALL) {
            newActiveFilters.tags = newActiveFilters.tags.filter(t => t !== tagToRemove.value)
          }
      ...
      removeTagByValue: (value: string, category?: string) => {
        set((state) => {
          const newTags = state.selectedTags.filter(t => 
            !(t.value === value && (!category || t.category === category))
          )
          ...
          if (category === TAG_CATEGORIES.TYPES) {
            ...
            newActiveFilters.categories = newActiveFilters.categories.filter(c => c !== value)
          } else if (category === TAG_CATEGORIES.FUZZY_SEARCH || category === TAG_CATEGORIES.SEARCH_ALL) {
            newActiveFilters.tags = newActiveFilters.tags.filter(t => t !== value)
          }
      ...
      setTags: (tags: SelectedTag[]) => {
        set((state) => {
          ...
          tags.forEach(tag => {
            if (tag.category === TAG_CATEGORIES.TYPES) {
              newActiveFilters.types.push(tag.value)
            } else if (tag.category === TAG_CATEGORIES.CATEGORIES) {
              newActiveFilters.categories.push(tag.value)
            } else if (tag.category === TAG_CATEGORIES.FUZZY_SEARCH || tag.category === TAG_CATEGORIES.SEARCH_ALL) {
              newActiveFilters.tags.push(tag.value)
            }
          })
      ...
      addTags: (tags: SelectedTag[]) => {
        set((state) => {
          ...
          const newTags = tags.filter(tag => 
            !existingTags.has(`${tag.value}-${tag.category}`)
          )
      ...
      hasTag: (value: string, category?: string) => {
        const state = get()
        return state.selectedTags.some(t => 
          t.value === value && (!category || t.category === category)
        )
      },
      ...
      getFuzzySearchTags: () => {
        const state = get()
        return state.selectedTags.filter(t => 
