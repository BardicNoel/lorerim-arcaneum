...
export const FloatingBuildButton = () => {
  ...
  const hasBuildContent = build.name || build.race || build.traits.length > 0 || build.stone || build.religion || build.skills.major.length > 0 || build.skills.minor.length > 0 || build.equipment.length > 0
      ...
      <Sheet open={isOpen} onOpenChange={setIsOpen}>
        ...
        <SheetContent side="right" className="w-[400px] sm:w-[540px] p-6">
          <SheetHeader>
            ...
            <SheetDescription>
              {hasBuildContent ? 'Your current character build status' : 'Create and manage your character build'}
            </SheetDescription>
          </SheetHeader>
            ...
            {hasBuildContent ? (
              <>
                ...
                {build.race && (
                  <div className="space-y-2">
                    <h3 className="text-sm font-medium text-muted-foreground">Race</h3>
                ...
                {build.stone && (
                  <div className="space-y-2">
                    <h3 className="text-sm font-medium text-muted-foreground">Birth Sign</h3>
                    <div className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                      {build.traits.map((traitId) => (
                        <div key={traitId} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                      ...
                      {build.skills.major.length > 0 && (
                        <div className="p-3 bg-muted/50 rounded-lg">
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <Badge variant="outline">{build.skills.major.length}</Badge>
                          </div>
                          <div className="flex flex-wrap gap-1">
                            {build.skills.major.map((skill) => (
                              <Badge key={skill} variant="secondary" className="bg-red-500 text-xs">
                                {skill}
                              </Badge>
                            ))}
                          </div>
                      ...
                      {build.skills.minor.length > 0 && (
                        <div className="p-3 bg-muted/50 rounded-lg">
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <Badge variant="outline">{build.skills.minor.length}</Badge>
                          </div>
                          <div className="flex flex-wrap gap-1">
                            {build.skills.minor.map((skill) => (
                              <Badge key={skill} variant="secondary" className="bg-blue-500 text-xs">
                                {skill}
                              </Badge>
                            ))}
                          </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                      {build.equipment.map((equipment) => (
                        <div key={equipment} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
              ...
              <div className="text-center py-8">
                ...
                <p className="text-muted-foreground mb-6">
                  Start building your character by selecting a race, traits, and other options.
                </p>
              </div>
            )}
          </div>
