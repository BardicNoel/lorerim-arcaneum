...
export const useCharacterBuild = () => {
  ...
  return {
    ...
    removeTrait: (trait: string) =>
      updateBuild({
        traits: build.traits.filter(t => t !== trait),
      }),

    addMajorSkill: (skill: string) =>
      updateBuild({
        skills: { ...build.skills, major: [...build.skills.major, skill] },
      }),
    removeMajorSkill: (skill: string) =>
      updateBuild({
        skills: {
          ...build.skills,
          major: build.skills.major.filter(s => s !== skill),
        },
      }),

    addMinorSkill: (skill: string) =>
      updateBuild({
        skills: { ...build.skills, minor: [...build.skills.minor, skill] },
      }),
    removeMinorSkill: (skill: string) =>
      updateBuild({
        skills: {
          ...build.skills,
          minor: build.skills.minor.filter(s => s !== skill),
        },
      }),
