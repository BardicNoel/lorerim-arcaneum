...
import { indexToSkillId, skillIdToIndex } from '../data/skillIndex'
import type { BuildState, CompressedPerks } from '../types/build'
...
export function encodePerks(perks: BuildState['perks']): CompactPerks {
  ...
  for (const [skillTreeId, selectedPerkEdids] of Object.entries(
    perks.selected
  )) {
    // Get the compact code for this skill tree
    const skillCode =
      SKILL_TREE_CODES[skillTreeId as keyof typeof SKILL_TREE_CODES]
    if (!skillCode) {
    ...
    for (const edid of selectedPerkEdids) {
      ...
      if (index !== -1) {
        ...
        console.warn(
          `Perk EDID not found in catalog: ${edid} for skill ${skillCode}`
        )
      }
    }
...
export function migrateToLegacyFormat(
  build: BuildState & { p?: CompactPerks }
): BuildState {
...
/**
 * Encodes legacy perk data to compressed format using skill indexes
 *
 * @param perks - Legacy perks object with selected and ranks
 * @returns Compressed perks object with skill indexes and perk indexes
 */
export function encodePerksToCompressed(
  ...
  const compressedPerks: CompressedPerks = {}

  for (const [skillTreeId, selectedPerkEdids] of Object.entries(
    perks.selected
  )) {
    // Convert skill ID to index
    const skillIndex = skillIdToIndex(skillTreeId)
    if (skillIndex === null) {
      console.warn(`Unknown skill tree ID: ${skillTreeId}`)
      continue
    }
    ...
    // Only add if there are selected perks
    if (indexes.length > 0) {
      compressedPerks[skillIndex] = indexes.sort((a, b) => a - b) // Sort for consistency
    }
  }

  return compressedPerks
}

/**
 * Decodes compressed perk data back to legacy format
 *
 * @param compressedPerks - Compressed perks object with skill indexes and perk indexes
 * @returns Legacy perks object with selected and ranks
 */
export function decodePerksFromCompressed(
  compressedPerks: CompressedPerks
): BuildState['perks'] {
  const selected: Record<string, string[]> = {}
  const ranks: Record<string, number> = {}

  for (const [skillIndexStr, indexes] of Object.entries(compressedPerks)) {
    const skillIndex = parseInt(skillIndexStr, 10)

    // Convert skill index to skill ID
    const skillTreeId = indexToSkillId(skillIndex)
    if (!skillTreeId) {
      console.warn(`Unknown skill index: ${skillIndex}`)
      continue
    }
...
/**
 * Migrates a build from any format to compressed format
 *
 * @param build - Build state to migrate
 * @returns Build state with compressed format
 */
export function migrateToCompressedFormat(build: BuildState): BuildState {
  // If already in compressed format, return as-is
  if ('n' in build && 'k' in build && 'a' in build) {
    return build
  }

  // Convert to compressed format using the build compression utilities
  const { legacyToCompressed } = require('./buildCompression')

  if ('perks' in build && !('p' in build)) {
    // Legacy format
    return legacyToCompressed(build)
  } else if ('p' in build && !('n' in build)) {
    // Compact format
    const { compactToCompressed } = require('./buildCompression')
